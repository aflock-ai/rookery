...
func TestDetectorFiresWhenJenkinsURLSet(t *testing.T) {
	reg := detection.NewRegistry()
	reg.Register(Name, detectorYAML)

	res := detection.RunPrePlanWith(reg, detection.PrePlan{
		Argv: []string{"echo", "hi"},
		Env:  map[string]string{"JENKINS_URL": "http://jenkins.local:8080/"},
		Cwd:  t.TempDir(),
	})
	if len(res.Fire) != 1 || res.Fire[0].Attestor != Name {
		t.Fatalf("expected jenkins to fire, got %+v", res.Fire)
	}
}
