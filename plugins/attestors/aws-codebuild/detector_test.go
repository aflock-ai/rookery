...
func TestDetectorFiresInCodeBuild(t *testing.T) {
	reg := detection.NewRegistry()
	reg.Register(Name, detectorYAML)

	res := detection.RunPrePlanWith(reg, detection.PrePlan{
		Argv: []string{"echo", "hi"},
		Env:  map[string]string{"CODEBUILD_PROJECT_NAME": "my-project"},
		Cwd:  t.TempDir(),
	})
	if len(res.Fire) != 1 || res.Fire[0].Attestor != Name {
		t.Fatalf("expected aws-codebuild to fire, got %+v", res.Fire)
	}
}
