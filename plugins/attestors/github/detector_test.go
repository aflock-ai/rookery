...
func TestDetectorFiresInGitHubActions(t *testing.T) {
	...
	if len(res.Fire) != 1 || res.Fire[0].Attestor != Name {
		t.Fatalf("expected github to fire, got %+v", res.Fire)
	}
}

func TestDetectorSkipsOutsideGitHubActions(t *testing.T) {
	reg := detection.NewRegistry()
	reg.Register(Name, detectorYAML)
	...
	if len(res.Fire) != 0 {
		t.Errorf("expected no fire, got %+v", res.Fire)
	}
}
