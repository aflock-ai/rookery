...
// Verify-only decoder for the historical v0.1 and v0.2 product predicate
// shapes. cilock verify can consume both pre-cutover wire formats; only the
// v0.3 attestor is wired as a producer.
//
// Why both v0.1 and v0.2 use one decoder:
//
// The pre-cutover product attestor's MarshalJSON returned the same byte
// shape — `map[string]attestation.Product` — regardless of which predicate
// type URI was published. v0.1 differed from v0.2 only at the in-toto
// Statement.Subject layer (per-file subjects in v0.1, a single hash-chain
// tree subject in v0.2). The predicate body is identical, so one decoder
// covers both.
//
...
//     engine's BFS can match historical attestations by per-file digest at
//     verify time — regardless of whether the original Statement.Subject
//     was per-file (v0.1) or a single tree-root subject (v0.2).
...
// because the legacy decoders register under distinct names
// (`product-v0.1`, `product-v0.2`).
...
const (
	...
	// that `cilock run --attestations product` selects the v0.3 producer,
	// never a legacy decoder.
	...
	// v0.2 shared the per-file map predicate body with v0.1; only the
	// in-toto Statement.Subject differed (hash-chain tree-root vs
	// per-file). The same LegacyDecoder handles both.
	LegacyV02Type = "https://aflock.ai/attestations/product/v0.2"
)

// errLegacyDecodeOnly is returned by Attest() on the legacy decoder so an
...
func init() {
	// Register both legacy types alongside the v0.3 producer.
	// RegisterAttestation is name+type keyed, so the v0.3 producer
	// (registered in product.go) and these decoders coexist without
	// collision.
...
// Attest refuses to produce a new attestation. v0.1 and v0.2 are no
// longer emittable; use v0.3.
...
// digest, regardless of whether the original Statement.Subject was
// per-file (v0.1) or a single tree-root subject (v0.2). The subject
// name format follows v0.1's `file:<path>` convention.
