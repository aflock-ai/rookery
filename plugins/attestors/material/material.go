...
// re-hashed on Linux.
func buildLeaves(mats map[string]cryptoutil.DigestSet) []MaterialLeaf {
	out := make([]MaterialLeaf, 0, len(mats))
	for path, ds := range mats {
		digest := extractSha256(ds)
		// extractSha256 returns "" if the DigestSet has no sha256
		// entry. Folding that absence into the leaf via an empty
		// digest is intentional: it produces a deterministic leaf
		// for unhashable files (e.g., sockets, broken symlinks) so
		// they still appear in the tree rather than silently
		// disappearing. The cilock walker normally guarantees a
		// sha256 entry for everything it returns, so this is a
		// defense-in-depth path, not a normal one.
		normalized := portableNormalize(path)
		out = append(out, MaterialLeaf{
			Path:       normalized,
			FileDigest: digest,
			LeafHash:   computeLeafHashHex(normalized, digest),
		})
	}
...
// computeLeafHashHex computes sha256(path || 0x00 || raw-file-digest)
// and returns the hex encoding. The file-digest input is hex-decoded
// into the raw 32 bytes before being concatenated; the published
// FileDigest field is hex but the leaf hashes RAW bytes so the
// encoding matches the on-disk artifact's bytes commitment.
//
// This is the canonical v0.3 leaf encoding. It MUST be byte-identical
// to the product attestor's encoding — TestV03_010 enforces that.
func computeLeafHashHex(path, fileDigestHex string) string {
	h := sha256.New()
	_, _ = h.Write([]byte(path))
	_, _ = h.Write([]byte{0})
	// hex.DecodeString returns nil bytes on a parse error. That's
	// fine: an unparseable digest still hashes deterministically
	// (under the empty byte slice), which keeps the tree well-defined
	// for the defense-in-depth path mentioned in buildLeaves. The
	// normal walk path always supplies a valid hex sha256.
	raw, _ := hex.DecodeString(fileDigestHex)
	_, _ = h.Write(raw)
...
func decodeLeafHashes(leaves []MaterialLeaf) ([][]byte, error) {
	...
	for i, l := range leaves {
		...
		if len(raw) != sha256.Size {
			return nil, fmt.Errorf("leaf %d (%s): hash length %d, want %d", i, l.Path, len(raw), sha256.Size)
		}
		out[i] = raw
	}
	return out, nil
