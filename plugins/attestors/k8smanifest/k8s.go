...
const Type = "https://aflock.ai/attestations/k8smanifest/v0.2"
...
// Attestor implements the Attestor interface for Kubernetes manifests.
...
func init() { //nolint:funlen // registration requires many options
	attestation.RegisterAttestation(
		...
		registry.StringSliceConfigOption(
			"ignore-annotations",
			"Additional ephemeral annotations to remove, e.g. aflock.ai/another-ephemeral",
			nil,
