...
# Set binary location (can be overridden with AFLOCK_BIN env var)
AFLOCK_BIN=${AFLOCK_BIN:-"aflock"}

# Check if binary is available (either in PATH or custom path)
if ! command -v "$AFLOCK_BIN" >/dev/null 2>&1; then
  echo "Error: 'aflock' binary not found."
  echo ""
  echo "To fix this:"
  echo "  1. Build from source using the builder."
  echo ""
  echo "  2. Or specify a custom binary path via environment variable:"
  echo "       export AFLOCK_BIN=./path/to/aflock"
  echo ""
  exit 1
fi

echo "Using binary: $(command -v $AFLOCK_BIN || echo $AFLOCK_BIN)"
...
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/plain-attestation.json" -- cat "$DEMO_DIR/plain-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on base64-encoded secret..."
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/base64-attestation.json" -- cat "$DEMO_DIR/base64-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on double base64-encoded secret..."
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/double-base64-attestation.json" -- cat "$DEMO_DIR/double-base64-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on URL-encoded secret..."
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/url-encoded-attestation.json" -- cat "$DEMO_DIR/url-encoded-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on hex-encoded secret..."
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/hex-encoded-attestation.json" -- cat "$DEMO_DIR/hex-encoded-secret.txt"

echo "==============================================================="
echo "Running secretscan attestor on mixed encoding secret..."
echo "==============================================================="
"$AFLOCK_BIN" run -a secretscan --log-level info -k testkey.pem -s test-step -o "$DEMO_DIR/mixed-encoding-attestation.json" -- cat "$DEMO_DIR/mixed-encoding-secret.txt"
...
extract_findings() {
  ...
  echo "=== $name Findings ==="
  jq -r '.payload' "$file" | base64 -d | jq '.predicate.attestations[] | select(.type=="https://aflock.ai/attestations/secretscan/v0.1") | .attestation.findings'
  echo
}
