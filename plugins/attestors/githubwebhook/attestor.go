...
const (
	Name    = "githubwebhook"
	Type    = "https://aflock.ai/attestations/githubwebhook/v0.1"
...
func (a *Attestor) Subjects() map[string]cryptoutil.DigestSet {
	...
	case EventPullRequestReview:
		if err := addPullRequestSubjects(a.Payload, toHash, subjects); err != nil {
...
func (a *Attestor) BackRefs() map[string]cryptoutil.DigestSet {
	...
	case EventPullRequest, EventPullRequestReview:
		pullRequest, err := PullRequestFromPayload(a.Payload)
		if err != nil {
			log.Debugf("could not get pull request from webhook payload: %v", err)
			break
		}
		...
		if len(pullRequest.MergeCommitSha) > 0 {
			subjects[fmt.Sprintf("mergecommitsha:%v", pullRequest.MergeCommitSha)] = cryptoutil.DigestSet{
...
func addPullRequestSubjects(payload map[string]any, toHash map[string]string, subjects map[string]cryptoutil.DigestSet) error {
