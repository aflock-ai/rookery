...
//
// DEVIATION from upstream go-piv v2.5.0: upstream's Close only disconnects the
// handle and releases the context, never ending the PC/SC transaction that
// Open() started with h.Begin(). On stacks that require SCardEndTransaction
// before SCardDisconnect, leaving the exclusive transaction open can lock the
// card/reader for the process. End the transaction first (best-effort: yk.tx is
// nil only if a future caller constructs a YubiKey without Open), then close
// the handle and context, preserving the first non-nil error.
...
func (c *client) Open(card string) (*YubiKey, error) {
	...
	tx, err := h.Begin()
	if err != nil {
		// DEVIATION from upstream go-piv v2.5.0: close h and ctx here. The
		// connection is SCardShareExclusive, so leaking them on a Begin failure
		// leaves the reader/card locked for the process and breaks retries.
		h.Close()
		ctx.Close()
		return nil, fmt.Errorf("beginning smart card transaction: %w", err)
	}
	if err := ykSelectApplication(tx, aidPIV[:]); err != nil {
		// DEVIATION from upstream go-piv v2.5.0: upstream closes only tx here,
		// leaking h and ctx (and the exclusive lock). Close all three.
		tx.Close()
		h.Close()
		ctx.Close()
