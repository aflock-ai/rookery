...
func TestResolveLoginCredentialFromToken(t *testing.T) {
	cred, err := resolveLoginCredential(newQuietCmd(), "https://platform.example.com", "tok-123", "", "")
	if err != nil {
...
func TestResolveLoginCredentialFromStdin(t *testing.T) {
	...
	cred, err := resolveLoginCredential(cmd, "https://platform.example.com", "-", "", "")
	if err != nil {
		t.Fatalf("resolveLoginCredential: %v", err)
	}
...
func TestResolveLoginCredentialEmptyTokenErrors(t *testing.T) {
	if _, err := resolveLoginCredential(newQuietCmd(), "https://platform.example.com", "   ", "", ""); err == nil {
