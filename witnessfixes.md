...
| M4 | **Symlink path traversal in file attestor (GHSA-v6px-jqx8-8xwj)** — `filepath.EvalSymlinks()` followed symlinks outside the working directory, allowing attestations to include digests of arbitrary out-of-tree files (e.g. `/etc/shadow`). Three sub-gaps: (a) per-file escaping symlinks, (b) the dir-hash branch followed escaping symlinks with no boundary, (c) the recursive call re-anchored the boundary to the per-call basePath so chained in-tree symlinks could move it. | `attestation/file/file.go`, `attestation/cryptoutil/dirhash_root.go` | Fail-closed by default in `RecordArtifacts`: (a) per-file escapes skipped; (b) `CalculateDigestSetFromDirWithinRoot` confines every dir-hash open beneath the root via `os.Root` (race-free, byte-identical hash for an equivalent tree); (c) the original attestation root is threaded immutably through symlink recursion. Opt-in `RecordArtifactsFollowingSymlinks` deliberately follows out-of-tree symlinks for build-input fidelity in TRUSTED trees only. |
