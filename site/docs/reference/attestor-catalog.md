...
| `command-run` (always run) | `https://aflock.ai/attestations/command-run/v0.2` | execute | argv, exit code, stdout/stderr digests, optional ptrace `openedfiles` and syscall records | [witness/command-run.md](https://github.com/in-toto/witness/blob/main/docs/attestors/command-run.md) |
...
| `configuration` | `https://aflock.ai/attestations/configuration/v0.2` | prematerial | Captures the CLI flags for the step (v0.2: config-file capture removed, args-only) | (cilock-native) |
