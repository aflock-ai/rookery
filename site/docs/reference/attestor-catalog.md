...
| `material` (always run) | `https://aflock.ai/attestations/material/v0.3` | material | Merkle root over the digests of all files in the working directory before the command runs. The per-file `leaves` (path, fileDigest, leafHash) are inlined in the attestation by default (#253), so the signed envelope is self-contained. A producer-side sidecar (`<outfile>.material.tree.json`) is also written for [`cilock prove`](../guides/prove-files-in-a-build) to emit standalone inclusion-proof attestations. | [material (v0.3)](../attestors/material) |
| `product` (always run) | `https://aflock.ai/attestations/product/v0.3` | product | Merkle root over the digests of files changed/created during execute (filtered by `--attestor-product-include-glob` / `--attestor-product-exclude-glob`). Per-file `leaves` are inlined in the attestation by default (#253); a producer-side sidecar `<outfile>.product.tree.json` is also written for `cilock prove`. | [product (v0.3)](../attestors/product) |
| `inclusion-proof` | `https://aflock.ai/attestations/inclusion-proof/v0.1` | producer-driven (`cilock prove`) | Signed RFC 6962 inclusion proof binding a single file's digest to a v0.3 product/material Merkle root. Generated on demand against a producer-side tree sidecar. | [inclusion-proof](../attestors/inclusion-proof) |
