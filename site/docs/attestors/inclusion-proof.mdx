...
Emits a signed inclusion proof binding a single file's digest to a Merkle tree's root. Generated on demand by [`cilock prove`](../guides/prove-files-in-a-build.md) against a producer-side tree sidecar; consumed by `cilock verify` when a downstream verifier needs to confirm a per-file claim against a v0.3 [product](./product.mdx) or material attestation.
...
- [Product attestor v0.3](./product.mdx) — the source of the Merkle root being proved against
...
- [Prove files in a build](../guides/prove-files-in-a-build.md) — producer-side workflow
