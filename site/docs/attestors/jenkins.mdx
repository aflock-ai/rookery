...
{/* GENERATED — do not edit. Source: rookery attestation/detection/docs/jenkins.doc.md. Regenerate: npm run gen */}
...
import meta from '@catalog/jenkins.json';
...
Use this attestor on any Jenkins-driven CI job where you want the resulting attestation to record which pipeline, job, build number, and agent node produced an artifact. Jenkins does not ship a native OIDC identity for build jobs, so this attestor only records self-reported environment variables — pair it with [`jwt`](./jwt.mdx) when a Jenkins plugin (for example, the OIDC Provider plugin) issues a verifiable token that can stand in as the functionary identity.
...
- [`jwt`](./jwt.mdx) for OIDC via Jenkins plugins
- Upstream: [witness/jenkins.md](https://github.com/in-toto/witness/blob/main/docs/attestors/jenkins.md)

<script type="application/ld+json" dangerouslySetInnerHTML={{__html: "{\"@context\":\"https://schema.org\",\"@type\":\"TechArticle\",\"headline\":\"jenkins — cilock attestor\",\"description\":\"The cilock jenkins attestor captures Jenkins build metadata from JENKINS_/BUILD_ environment variables and signs it into in-toto evidence, binding an artifact to the pipeline run that produced it.\",\"url\":\"https://cilock.aflock.ai/attestors/jenkins\"}"}} />

<GeneratedFooter name="jenkins" />
