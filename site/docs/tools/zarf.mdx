...
{/* GENERATED — do not edit. Source: rookery attestation/detection/docs/zarf.doc.md. Regenerate: npm run gen */}
...
import meta from '@catalog/zarf.json';
...
- [`command-run` attestor](../attestors/command-run.mdx) — records the literal zarf argv + exit code
- [`product` attestor](../attestors/product.mdx) — how the `zarf-package-*.tar.zst` tarball lands in the Merkle tree
- [`material` attestor](../attestors/material.mdx) — snapshots `zarf.yaml` + sources before the build
- [`syft` tool](./syft.mdx) — generate a cilock-parsed CycloneDX/SPDX SBOM of the built package
...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: "{\"@context\":\"https://schema.org\",\"@type\":\"TechArticle\",\"headline\":\"Zarf — cilock tool integration\",\"description\":\"Run Zarf under cilock so `zarf package create` — the air-gap package build — is captured as a signed run where the package tarball becomes a real v0.3 product and the literal zarf argv is recorded in command-run/v0.2.\",\"url\":\"https://cilock.dev/tools/zarf\"}"}} />

<GeneratedFooter name="zarf" />
