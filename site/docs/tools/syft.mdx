...
{/* GENERATED — do not edit. Source: rookery attestation/detection/docs/syft.doc.md. Regenerate: npm run gen */}
...
import meta from '@catalog/syft.json';
...
- [`sbom` attestor](../attestors/sbom.mdx) — the underlying ingestion path, predicate-type switching, and SBOM format detection
- [`product/v0.3` attestor](../attestors/product.mdx) — how syft's output file lands in the Merkle tree as a real product
- [`command-run/v0.1` attestor](../attestors/command-run.mdx) — what records the literal syft argv
...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: "{\"@context\":\"https://schema.org\",\"@type\":\"TechArticle\",\"headline\":\"Syft — cilock tool integration\",\"description\":\"Generate signed CycloneDX or SPDX SBOMs with cilock — Anchore's Syft runs under `cilock run` so the SBOM is captured as a real v0.3 product, hashed into a Merkle tree, and parsed by the `sbom` attestor.\",\"url\":\"https://cilock.aflock.ai/tools/syft\"}"}} />

<GeneratedFooter name="syft" />
