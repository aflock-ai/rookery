...
{/* GENERATED — do not edit. Source: rookery attestation/detection/docs/checkov.doc.md. Regenerate: npm run gen */}
...
import meta from '@catalog/checkov.json';
...
- [`sarif` attestor](../attestors/sarif.mdx) — the underlying ingestion path and predicate schema
- [Verify in a release gate](../guides/verify-in-a-release-gate.md) — how a `sarif/v0.1` attestation feeds policy at deploy time
- [The spine of the graph](../concepts/the-spine-of-the-graph.md) — how subject digests chain `sarif` to `material`, `product`, and `git`
- [Tools index](./index.md)

<script type="application/ld+json" dangerouslySetInnerHTML={{__html: "{\"@context\":\"https://schema.org\",\"@type\":\"TechArticle\",\"headline\":\"Checkov — cilock tool integration\",\"description\":\"Scan Terraform, CloudFormation, Kubernetes, Helm, and other IaC with Checkov under cilock — every misconfig becomes a signed v0.3 SARIF attestation.\",\"url\":\"https://cilock.dev/tools/checkov\"}"}} />

<GeneratedFooter name="checkov" />
