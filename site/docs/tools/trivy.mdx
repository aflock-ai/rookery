...
Yes — through the generic [`sarif` attestor](../attestors/sarif.mdx) (documented on this page). Run Trivy with `--format sarif` and cilock captures the report as a signed `sarif/v0.1` attestation, the same uniform predicate it produces for every SARIF-emitting scanner (Grype, Semgrep, gosec, …).
