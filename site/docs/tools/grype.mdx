...
{/* GENERATED — do not edit. Source: rookery attestation/detection/docs/grype.doc.md. Regenerate: npm run gen */}
...
import meta from '@catalog/grype.json';

# `grype` integration

Grype accepts container images, directories, archives, and pre-existing Syft SBOMs as scan targets. A native Grype attestor is in development (tracked in the [tools catalog](./)); today the supported route is the [`sarif`](../attestors/sarif.mdx) attestor.
...
See the [`sarif` attestor reference](../attestors/sarif.mdx) for the full predicate schema and additional Rego patterns.
...
- [`sarif` attestor reference](../attestors/sarif.mdx) — predicate schema, Rego patterns, the `.report` wrapper
- [`product/v0.3` attestor](../attestors/product.mdx) — how Grype's SARIF gets captured as a real product
- [`command-run/v0.1` attestor](../attestors/command-run.mdx) — how the literal `grype` argv is recorded
...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: "{\"@context\":\"https://schema.org\",\"@type\":\"TechArticle\",\"headline\":\"Grype — cilock tool integration\",\"description\":\"Scan images or directories for vulnerabilities with Grype under cilock — the SARIF report becomes a signed v0.3 attestation parsed by the rookery sarif attestor, byte-identical to what Grype produced.\",\"url\":\"https://cilock.aflock.ai/tools/grype\"}"}} />

<GeneratedFooter name="grype" />
