...
func EnableVerity(path string, hashAlg uint32) error {
	...
	f, err := os.OpenFile(path, os.O_RDONLY, 0) //nolint:gosec // G304: path is the caller's intentional target
	if err != nil {
	...
	_, _, errno := unix.Syscall(unix.SYS_IOCTL,
		...
		uintptr(unsafe.Pointer(&arg)), //nolint:gosec // G103: kernel ioctl arg-struct, intentional unsafe.Pointer
	)
	if errno != 0 {
...
func MeasureVerity(path string) ([]byte, uint16, error) {
	...
	_, _, errno := unix.Syscall(unix.SYS_IOCTL,
		...
		uintptr(unsafe.Pointer(&d)), //nolint:gosec // G103: kernel ioctl arg-struct, intentional unsafe.Pointer
	)
	if errno != 0 {
