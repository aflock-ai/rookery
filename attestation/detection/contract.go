...
func validateOutputContract(c *OutputContract, name string) error { //nolint:gocognit,gocyclo,funlen,nestif // exhaustive field-by-field contract validator: a flat sequence of independent build-time consistency checks; splitting it would scatter the contract's invariants across helpers without reducing real complexity.
	if c == nil {
		return nil
	}
