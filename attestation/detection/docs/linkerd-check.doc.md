...
sidebar_position: 20
examples_repo: tool-linkerd-check
---
...
The single `sh -c` wrapper is the same pattern as [hadolint](./hadolint), [govulncheck](./govulncheck), and [falco](./falco) — tools that need stdout-to-file conversion to be hashed by the product attestor. The shell redirect is a one-shot conversion, not the cp antipattern.
...
- [`linkerd-check` attestor](../attestors/linkerd-check) — the underlying ingestion path
