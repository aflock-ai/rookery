...
sidebar_position: 1
examples_repo: tool-trivy-sarif
---
...
Trivy exits non-zero when it finds vulnerabilities at or above its severity threshold. The validated example uses Trivy's default exit-code behavior; cilock surfaces that exit code in the `command-run/v0.2` predicate, so policies can reason about scan results without re-parsing.

## What gets captured

A single run with `--attestations sarif,environment,git` produces these predicate types in the signed envelope:

- `https://aflock.ai/attestations/command-run/v0.2` — the actual `argv`, exit code, and traced syscalls of `trivy`.
