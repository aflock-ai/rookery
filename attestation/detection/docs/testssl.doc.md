...
| `https://aflock.ai/attestations/command-run/v0.2` | literal `testssl.sh --jsonfile-pretty testssl.json --quiet <target>` argv + exit code |
...
- **Hostnames vs `host:port`.** testssl defaults to port 443. For non-HTTPS TLS services (LDAP-S, SMTPS, IMAP-S) use `host:port`. STARTTLS protocols need `--starttls <protocol>` — e.g. `--starttls smtp` for port 25 STARTTLS. The cilock envelope captures the exact `host:port` or `--starttls` argv in `command-run/v0.2`.
...
Yes. Wrap `testssl.sh --jsonfile-pretty <out>.json --quiet <target>` with `cilock run --attestations sarif,environment,git`. The testssl JSON becomes a signed v0.3 attestation under `https://aflock.ai/attestations/sarif/v0.1`, the literal `testssl.sh` argv is captured in `command-run/v0.2`, and the JSON file is hashed into the v0.3 Merkle tree as a real product.
