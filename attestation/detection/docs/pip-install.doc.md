...
Each `PackageInfo` carries `name`, `version`, `installType`, `location`, `requires`, `requiredBy`, `homePage`, `author`, `license`, `hasSetupPy`, `hasCmdClass`, and `installerLog`. The non-name/version fields are populated from `pip show <name>` output, parsed line-by-line.
...
After a Python build or install step, to record what actually ended up on disk and to surface a few classes of supply-chain red flag in the same predicate. Pair with [`lockfiles`](./lockfiles) (pre-material pin) and [`sbom`](./sbom) (resolved tree) for full coverage; this attestor is the only one that observes the post-install state of `site-packages`.
