...
If HEAD cannot be resolved, the attestor fails closed — unless the repository is provably unborn (no refs and no objects: a brand-new repo with no commits), in which case it emits an empty predicate rather than failing the run.
...
## Keyless commit and tag signing

The same `cilock` binary implements Git's standard X.509 signing protocol. Configure the current repository once:
...
CI/lock defaults to the hosted TestifySec platform. It exchanges the current stored session or registered workflow identity for a short-lived Fulcio certificate, signs with an ephemeral in-memory key, and requires an RFC 3161 timestamp from the platform TSA. Private installations use the same protocol with the organization's TestifySec appliance. No signing flag, platform flag, separate `gitsign` executable, or credential-cache daemon is required.

This covers the everyday gitsign-compatible Git surface: signed commits, signed annotated tags, `git log --show-signature`, `git verify-commit`, and `git verify-tag`. CI/lock is designed for an organization's private trust infrastructure, so it uses platform discovery and pinned private trust instead of gitsign's public-community TUF/Rekor configuration. CI/lock's existing `run`, `attest`, and `verify` commands remain the attestation and policy interface; it does not duplicate gitsign's source-predicate subcommands.
...
- **Signing and attesting are distinct.** The Git signature authenticates the proposed commit or tag. The `git` attestor records repository state inside signed in-toto evidence. Pushgate policy can require both without treating either as a substitute for the other.
