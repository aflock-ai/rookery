...
sidebar_position: 8
examples_repo: tool-kubescape-sarif
---
...
`command-run/v0.2` records the literal `["sh", "-c", "kubectl get … && kubescape scan …"]` argv, so the dump-then-scan recipe is fully captured.
...
|---|---|
| `https://aflock.ai/attestations/command-run/v0.2` | The argv cilock actually exec'd |
...
1. `command-run/v0.2` records the real argv (`["kubescape", "scan", "framework", "nsa", ...]`), not `cp`.
...
Yes, via the snapshot pattern: `sh -c 'kubectl get … -o yaml > snapshot.yaml && kubescape scan … snapshot.yaml'`. The kubectl dump captures real cluster state (live `SecurityContext`, RBAC, NetworkPolicies); Kubescape then scans the YAML dump as a local-file context and emits SARIF. `command-run/v0.2` records the full `sh -c` argv so the recipe is auditable.
...
- [How cilock policy works](../guides/policy) — how the SARIF predicate is gated at verify time
- [Attestation graph + back-refs](../concepts/attestation-graph) — how the scan links via subject digests to the artifact
